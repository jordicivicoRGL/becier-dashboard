...
**EXPERTO 4 — REVISOR DE BRIEF**
Actúa antes de mostrar las propuestas a Jordi. Verifica que cada AD ataca un ángulo realmente distinto (no la misma idea con otras palabras), que el hook se puede decir o leer en 3 segundos, y que el cierre es específico al contexto del anuncio (no una frase genérica). Si algo falla, lo corrige directamente antes de la pausa de validación. Nada llega a Jordi sin pasar por este experto.
...
*Solo si Jordi eligió "Google Sheets" o "Ambos" como output. Si eligió "Solo en el chat", el skill termina tras la validación de Fase 2.*
...
Si `clients/[cliente].md` **no existe**, pregunta primero: *"¿Quieres que guarde el perfil de [cliente] para futuras propuestas?"* — y créalo solo si confirma.
